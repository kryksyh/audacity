...
namespace {
static constexpr int DBLOG_STEREO_MIN_ADJACENT_FULL_STEPS_HEIGHT = 17;
static constexpr double DBLOG_STEREO_MIN_CHANNEL_HEIGHT = 40.0;
}

DbLogStereoRuler::DbLogStereoRuler()
    : DbLogBaseRuler(DbLogRulerUiSettings { DBLOG_STEREO_MIN_ADJACENT_FULL_STEPS_HEIGHT })
{
}
...
std::vector<TrackRulerFullStep> DbLogStereoRuler::fullStepsForHalfWave() const
{
    ...
    for (size_t channel = 0; channel < channelHeights.size(); ++channel) {
        if (channelHeights[channel] < DBLOG_STEREO_MIN_CHANNEL_HEIGHT) {
