...
static constexpr int LINEAR_BASE_MIN_ADJACENT_FULL_STEPS_HEIGHT = 20;
static constexpr int LINEAR_BASE_MIN_ADJACENT_SMALL_STEPS_HEIGHT = 10;
...
std::pair<double, double> LinearBaseRuler::stepsIncrement(double height) const
{
    ...
    for (const auto& stepInc : STEP_INCREMENT) {
        ...
        if ((valueToPosition(0.0, height) - valueToPosition(fs * factor, height) >= LINEAR_BASE_MIN_ADJACENT_FULL_STEPS_HEIGHT)
            && (valueToPosition(0.0, height) - valueToPosition(ss * factor, height) >= LINEAR_BASE_MIN_ADJACENT_SMALL_STEPS_HEIGHT)) {
