...
namespace {
static constexpr int DBLINEAR_STEREO_MIN_FULL_STEP_TO_INF_HEIGHT = 14;
static constexpr int DBLINEAR_STEREO_MIN_ADJACENT_STEPS_HEIGHT = 10;
static constexpr int DBLINEAR_STEREO_MIN_ADJACENT_SMALL_STEPS_HEIGHT = 5;
static constexpr int DBLINEAR_STEREO_MIN_FULL_STEP_TO_ZERO_HEIGHT = 10;
static constexpr int DBLINEAR_STEREO_MIN_CHANNEL_HEIGHT = 30;
static const std::vector<int> DBLINEAR_STEREO_FULL_STEP_SIZES = { 1, 2, 3, 6, 9 };
static constexpr double DBLINEAR_STEREO_HALF_WAVE_OFFSET_DB = 6.0;
}

DbLinearStereoRuler::DbLinearStereoRuler()
    : DbLinearBaseRuler(DbLinearRulerUiSettings { DBLINEAR_STEREO_MIN_ADJACENT_STEPS_HEIGHT, DBLINEAR_STEREO_MIN_FULL_STEP_TO_INF_HEIGHT, DBLINEAR_STEREO_MIN_FULL_STEP_TO_ZERO_HEIGHT,
                                                  std::vector<int>(DBLINEAR_STEREO_FULL_STEP_SIZES.begin(), DBLINEAR_STEREO_FULL_STEP_SIZES.end()) })
{
}
...
std::vector<TrackRulerFullStep> DbLinearStereoRuler::fullStepsForHalfWave() const
{
    ...
    if (m_collapsed) {
        const double halfMaxDisplayValueDB = maxDisplayValueDB - DBLINEAR_STEREO_HALF_WAVE_OFFSET_DB;
    ...
    for (size_t channel = 0; channel < channelHeights.size(); ++channel) {
        if (channelHeights[channel] < DBLINEAR_STEREO_MIN_CHANNEL_HEIGHT) {
            const double halfMaxDisplayValueDB = maxDisplayValueDB - DBLINEAR_STEREO_HALF_WAVE_OFFSET_DB;
...
std::vector<TrackRulerSmallStep> DbLinearStereoRuler::smallSteps() const
{
    ...
    for (size_t channel = 0; channel < 2; ++channel) {
        ...
        for (const auto& [startValue, endValue] : fullStepRanges) {
            ...
            if (((firstPos - secondPos) / (endValue - startValue)) > DBLINEAR_STEREO_MIN_ADJACENT_SMALL_STEPS_HEIGHT) {
